...
for item in component_code_list:
    for row in ws1.iter_rows("C"):
        for cell in row:
            ...
            if cell.value == item[1]:
                match_in_loop_new_detect = match_in_loop_new_detect + 1
                component_code_cnt = component_code_cnt+1
                
                
                # print("STT in second file: "+ str(component_code_match_row_index))
                match_indices.append(component_code_match_row_index)
    ...
    if match_in_loop_new_detect != 0:
        print("Detect " + str(match_in_loop_new_detect) + " matching in second file With these row corresponding indices: ")
        #print(":")
        print(match_indices)
        match_indices = []
    else:
    ...
    for row in ws2.iter_rows("F"):
        for cell in row:
            if cell.value == item[3]:
                ...
                print("Process component: " + str(add_cnt) + " / " + str(len(list_has_foreign_name)))
